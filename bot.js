...
mongoose.connect(config.testDatabase).then(console.log(`Connected to database!`));
const User = require("./models/user.js");

//imports functions
const functions = require("./assets/functions.js")(bot);
...
// function autoDelMessages(time) {
// 	message.channel.fetchMessages({limit: 1})
// 	.then(function(mList) {
// 		message.delete([time], mList);
// 	}, function(err){console.log("Could not delete messages.")})
// }
// 	if(message.channel.id === config.verifiedChannel) {
...
// 		autoDelMessages(300);
// 	}
// 		//could definitely make this cleaner ^
// }
...
.setTimestamp();
	...
	//Join cooldown. Launches the following two functions after specified time:
	setTimeout(function() {
		member.addRole(config.verifiedRole).catch(console.error);
	}, 30000);
